...
def bradley_terry_probs(elo_home, elo_away, home_adv=60.0, k_draw=0.22):
    """
    Probabilidades 1X2 desde gap Elo con empates (heurística simple).
    """
    gap = (elo_home + home_adv) - elo_away
    # Mapear gap a logits (escala 400 ~ dos divisiones)
    z_home = gap / 120.0
    z_away = -gap / 120.0
    z_draw = 0.0
    p = softmax3(np.array([z_home, z_draw, z_away]))
    # Aumentar prob de empate si gap pequeño
...
def ensemble_predict(lh, la, elo_h, elo_a, weights=(0.4, 0.35, 0.25), rho=0.12):
    """
    Combina Poisson, Dixon–Coles y Bradley–Terry/Elo.
    weights: (w_poisson, w_dc, w_elo)
    """
    # Poisson
